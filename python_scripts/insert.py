...
query_1 = ("INSERT INTO star_share_report "
        "(app_name, one_star, two_star, three_star, four_star, five_star, count, user_id) "
        "VALUES (%(APP_NAME)s, %(ONE_STAR)s, %(TWO_STAR)s, %(THREE_STAR)s, %(FOUR_STAR)s, %(FIVE_STAR)s, %(COUNT)s, %(user_id)s)")
...
for idx in range(len(subject_name)):
    query_2 = ("INSERT INTO star_report "
        "(app_name, subject, avg_star, count, user_id) "
        "VALUES (%(APP_NAME)s, " + '"' + subject_name[idx] + '"' + ", %(" + subject_name[idx] + ")s, %(" + subject_name_cnt[idx] + ")s, %(user_id)s)")
